...
namespace EvaluacionCore.Application.Features.EvalCore.Queries.GetMarcacionAsistenciaAsync;

public record GetMarcacionAsistenciaAsyncQuery(string Suscriptor, string fechaDesde, string fechaHasta, string Udn, string Area, string Departamento, string FiltroNovedades,string identSession,Guid? idCanal) : IRequest<ResponseType<List<EvaluacionAsistenciaResponseType>>>;

public class GetMarcacionAsistenciaAsyncHandler : IRequestHandler<GetMarcacionAsistenciaAsyncQuery, ResponseType<List<EvaluacionAsistenciaResponseType>>>
{
    ...
    public GetMarcacionAsistenciaAsyncHandler(IEvaluacion repository,
    ...
    async Task<ResponseType<List<EvaluacionAsistenciaResponseType>>> IRequestHandler<GetMarcacionAsistenciaAsyncQuery, ResponseType<List<EvaluacionAsistenciaResponseType>>>.Handle(GetMarcacionAsistenciaAsyncQuery request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                var novedadMarcacionWeb = await _repoMarcacionAsync.ConsultaAsistencia(col.Identificacion, request.FiltroNovedades, fechaDesde, fechaHasta, cancellationToken);
