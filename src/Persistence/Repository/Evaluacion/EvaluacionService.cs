...
public class EvaluacionService : IEvaluacion
{
    ...
    public async Task<List<ControlAsistenciaType>> ConsultaAsistencia(string CodigoBiometrico, DateTime fechaDesde, DateTime fechaHasta)
    {
        ...
        try
        {
            ...
            string query = "SELECT * FROM GRIAMSE.dbo.VV_CASISTENCIA WHERE CODIGOBIOMETRICO = '" + CodigoBiometrico + "' AND FECHAASGINACION BETWEEN '" + fechaDesde.ToString("yyyy/MM/dd HH:mm:ss") + "' AND '" + fechaHasta.ToString("yyyy/MM/dd HH:mm:ss") + "' ORDER BY FECHAASGINACION ASC";
